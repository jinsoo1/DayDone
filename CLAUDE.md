...
> **준비금(FUTURE_PREPARE)의 위치 (v1.4.3)**: `CalculateDailyBudgetUseCase`에서 **오늘 날짜 준비금은 `todaySpent`(오늘 쓴 돈·초과 판정)가 아니라 `budgetBeforeToday`(권장 금액의 분자)에서 뺀다.** 생활비에서 빠지는 건 같지만 "오늘 권장을 넘겼다"로 잡히지 않고 오늘 권장 자체가 그만큼 낮아진다. `remainingPureBudget`·`tomorrowRecommended` 값은 변하지 않는다(둘 다 전체를 빼므로). 지난 날짜 준비금은 원래대로 `pastSpent`. 무지출 판정·저녁 알림 ②가 GENERAL만 보는 것과 같은 방향 — **준비금은 소비가 아니다.** 테스트 4개로 고정.
>
...
- ⚠️ **레코드가 하나라도 생기면 폴백은 사실상 죽는다**(이월이 항상 이김). 그래서 "기본 수입만 갱신하는" 입구를 만들면 그 화면은 반영이 안 되는 것처럼 보인다 — 설정 탭 월 수입이 이 함정에 빠졌었고, 지금은 현재 기간 레코드를 함께 쓴다(§13 "설정 탭 월 수입" 참고).
...
- 날짜 칩: 오늘 기준 앞뒤 며칠(`GetTodayDateChipsUseCase.DAYS_BEFORE/AFTER` = 3, 총 7개). 지출/예정 차감 있는 날 표시.
- **상단 권장 금액 카드는 항상 오늘 기준 유지.** 날짜 칩을 눌러도 하단 내역만 선택 날짜로 바뀐다.
- 라벨: 과거 "7월 1일 내역" / 오늘 "오늘 내역" / 미래 "7월 3일 예정".
- ⚠️ **칩은 예산 기간 경계를 넘어간다** — 기간 첫날엔 지난 기간의 며칠, 마지막 날엔 다음 기간의 며칠이 칩으로 뜬다. 그래서 조회는 **기간이 아니라 `min(기간 시작, 오늘−3) ~ max(기간 끝, 오늘+3)`**(지출·추가수익), 예정 차감은 **이웃 기간까지 함께 계산**(`neighborPeriodsForChips`)한다. 표시(칩 마커·선택 날짜 내역)만 이 넓은 범위를 쓰고, **예산 계산은 `loadToday`에서 `budgetPeriod.contains`로 다시 잘라 이번 기간 것만** 쓴다 — 안 자르면 지난 기간 지출이 `pastSpent`에 섞여 권장 금액이 틀어진다(`CalculateDailyBudgetUseCase`는 기간으로 필터하지 않는다). v1.4.1까지는 기간으로 잘라 읽어서 1일에 지난달 29~31일 칩이 전부 "내역이 없어요"로 보였다.
...
진입 카드 2종(요약 카드 아래): **리포트**(진행 중=이번 기간 / 지난 달=결산, `MonthViewMode`로 분기) · **내역 전체 보기**(모든 모드, 아래 참고).
...
> - **격자는 예산 기간을 넘지 않는다** — 주 앞뒤 여백은 `null` 셀(그려지지도, 눌리지도 않음)이고 셀은 기간 안 날짜만 만든다. `effectiveSelected`가 기간 밖 선택을 되돌리고 월 이동 시 선택을 초기화하므로 **표시 범위 = 조회 범위**가 유지된다. 여백을 이웃 기간 날짜로 채우려면 오늘 탭 칩처럼 조회 범위도 함께 넓혀야 한다(그러면 "이 달 합계"까지 다시 잘라야 해서 비용이 크다 — 그래서 안 채운다).
...
- **지난 기간 대비 (구현됨, v1.1)**: `MonthlyReport.previous`(`PreviousComparison`) — **전부 "같은 시점(dayIndex)까지"끼리 비교**(기간 전체와 비교하면 진행 중엔 항상 "덜 씀" 왜곡). 표시 2곳: ① 페이스 카드 아래 한 줄 "지난 기간 이맘때보다 N원 덜/더 쓰고 있어요"(결산은 "지난 기간보다 ~썼어요", 덜 쓸 때만 성공 색) ② 미니 지표(하루 평균·무지출)에 **"지난 기간 이맘때 {값}"** 보조 줄(결산은 "지난 기간 {값}") + 미니 지표 아래 한 줄로 그 뜻을 설명("지난 기간의 같은 N일째까지만 모아본 값"). 보조 줄 문구를 "지난 기간 {값}"으로만 두면 지난 기간 **전체** 평균으로 오해된다 — 비교 시점을 문구에 드러낼 것. 세 카드는 보조 줄 유무·줄바꿈이 달라 높이가 어긋나므로 `IntrinsicSize.Min` + `fillMaxHeight`로 맞춰뒀다. **숨김 조건**: 지난 기간이 없거나 부분 기록(최초 지출 > 지난 기간 시작일)이면 previous=null — 어긋난 비교보다 없는 게 §13. 무지출 비교는 같은 확정 일수(진행 중=dayIndex-1일, 결산=dayIndex일)로 창을 맞춤. ViewModel이 지난 기간 지출을 함께 관찰(중첩 combine).
...
### 기간 내역 화면 ("내역 전체 보기", 구현됨)

한 예산 기간의 수입·지출을 **날짜별로 묶어 쭉 이어보는** 읽기 전용 화면(`LedgerRoute`, 별도 라우트 `ledger?month=yyyy-MM`, 하단바 숨김). 월 탭은 "선택한 하루"만 보여줘서 기간 전체를 훑을 화면이 없었다.

- 진입 **2곳**:
  - **오늘 탭 "오늘 내역" 섹션 헤더의 "전체 보기 ›"** — 하루치만 보여주는 섹션에서 기간 전체로 넘어가는 게 가장 자연스럽고, `month` 없이 열어 **현재 기간**을 본다. ⚠️ 새 카드·배너를 만들지 않고 **기존 섹션 헤더에 텍스트 링크**만 얹은 이유는 오늘 탭 과밀(배너 3종) — 큰 지출 안내를 오늘 탭에 안 넣은 것과 같은 판단이다.
  - **월 탭 요약 카드 아래 🧾 "내역 전체 보기"** — 리포트와 달리 **지난/이번/다가올 달 모두** 열린다(보고 있는 달의 `anchorMonthValue`를 그대로 넘김). 지난 달 내역으로 가는 유일한 경로.
  - 화면 안에 월 이동은 없다(월 탭에서 옮기고 다시 들어온다).
- 구성: 상단 요약(기간 · 정렬 토글 · **"남은 생활비"를 headlineSmall로 크게** · 그 아래 내림 계산 4줄: 월 예산 / 들어온 돈 / 쓴 돈 / 저축·고정비) → 날짜 구역들.
- ⚠️ **남은 금액은 월 탭 예산 요약과 같은 식**(`월 예산 + 추가수익 − 저축·고정비 − 지출`). 오늘 탭의 `remainingPureBudget`(미래 날짜 지출 제외)과는 미묘하게 다를 수 있는데, 이 화면은 **미래 날짜 지출까지 목록에 보여주므로** 화면에 보이는 줄들의 합과 맞는 쪽을 택했다. 라벨은 오늘이 그 기간 안일 때만 "남은 생활비", 지난/다가올 기간이면 "남은 금액"(지난 기간에 "남은 생활비"는 뜻이 안 맞음). 초과(음수)면 `−`를 붙여 사실대로 쓰고 색만 `spendText` — §13대로 실패로 표현하지 않는다.
- **정렬**: 기본 오름차순(기간 첫날 → 마지막 날), 토글로 내림차순. **DataStore(`ledger_ascending`)에 저장**(v1.4.3) — 같은 카드의 저축·고정비 숨김이 저장되는데 정렬만 나갔다 오면 초기화되는 불일치가 있었다. 은행 앱은 최신순이 기본이라 그게 익숙한 유저는 매번 뒤집어야 했다. 정렬·숨김 모두 조회 flow **밖**에서 `combine`하므로 값만 바뀌면 재계산되고 DB는 다시 읽지 않는다.
- 하루 안 순서는 **수입 → 지출·준비금 → 저축·고정비**. 기록이 하나도 없는 날은 **구역을 만들지 않는다**(빈 날로 스크롤이 늘어나지 않게).
- **훑기(scan) 우선 레이아웃 — 1차 구현(날짜별 회색 카드 나열)이 "눈에 안 들어온다"는 피드백으로 다시 잡은 규칙:**
  - **하루 = 카드 하나**, 그리고 그 카드의 헤더가 `stickyHeader`. LazyColumn 에선 헤더와 항목이 별개 아이템이라 하나의 Composable 로 감쌀 수 없으므로, **배경 모양으로 카드를 이어 붙인다** — 헤더는 위 모서리만 둥글게, 마지막 항목 줄은 아래 모서리만 둥글게, 그 뒤에 배경 밖 `Spacer`로 카드 간격. 색은 `ledgerCardColor()` 한 곳에서 가져와 헤더와 줄이 반드시 같은 색이게 한다.
  - ⚠️ **스티키 헤더는 불투명해야 한다.** 카드색이 반투명 틴트(`surfaceVariant` alpha 0.4)라서 틴트만 주면 아래로 지나가는 항목이 헤더를 통해 비쳐 보인다 → `background(colorScheme.background)`를 먼저 깔고 그 위에 틴트를 얹는다(둥근 모서리 바깥은 페이지 배경과 같은 색이라 안 보인다).
  - 날짜는 그 날 항목이 지나갈 때까지 상단에 붙어 있어야 긴 목록에서 길을 잃지 않는다.
  - 날짜 표기는 **큰 일(日) 숫자 + 작은 요일**(`dayText`/`weekText`). "9월"을 30번 반복하지 않고, 기간이 두 달에 걸칠 때 **달이 바뀌는 지점에만** `monthLabel`을 띄운다.
  - 그날 **지출 합계가 그 줄에서 가장 큰 숫자**(titleMedium Bold). 항목 금액은 bodyMedium — 위계가 있어야 눈이 어디 내릴지 안다.
  - 금액은 전부 `fontFeatureSettings = "tnum"`(tabular) — 자릿수가 흔들리지 않아 세로로 훑힌다.
- 색 (⚠️ 1차 구현에서 **반대로** 했다가 고친 부분): **지출 줄은 기본색**(`onSurface`), 색은 예외에만 — 수입 `successText`(+왼쪽 3dp 초록 막대), 저축·고정비 `onSurfaceVariant`, 그리고 **그날/기간 지출 합계에만** `DayDoneAccent.spendText`. 줄의 95%가 지출인데 전부 빨강으로 칠하면 **빨강이 아무것도 구분하지 못한다**(대비 소멸). `spendText`는 경고색이 아니라 구분색이라 경고 빨강보다 톤을 눌러뒀다(§13 — 위젯에서 빨간 숫자를 뺀 것과 같은 이유). 화면에서 `Color(0xFF…)` 하드코딩 금지 규칙대로 `AccentColors.kt`에만 둔다.
...
- ✅ (v1.4.3) 준비하기는 `LocalDate.now()` 지출을 만드는데, 1.4.2까진 `CalculateDailyBudgetUseCase`가 준비금도 오늘 지출로 세서 **옮긴 직후 카드가 "오늘 권장보다 더 썼어요"로 바뀌었다** — 앱이 권한 행동을 하자마자 혼내는 모양. 이제 오늘의 FUTURE_PREPARE는 `todaySpent`가 아니라 `budgetBeforeToday`에서 빠진다(§7 참고). 코어 한 곳이라 위젯·살까 말까·보류함이 함께 맞춰진다.
...
설계: `docs/v1.4-design.md`. **Room 무변경**, 설정은 DataStore(`NotificationSettingsDataSource`).

- ⚠️ **"권한 0개" → "켜지 않으면 권한 0개"**. 기본값 전부 off, 온보딩에서 안 묻고 **설정에서 켤 때만** `POST_NOTIFICATIONS` 요청(거부 시 스위치 되돌림). 스토어·홍보 문구도 이 표현으로 정정됨 — **다시 "권한 0개"로 쓰지 말 것.** `RECEIVE_BOOT_COMPLETED`도 추가됐지만 민감 권한 아님. **`SCHEDULE_EXACT_ALARM`은 절대 쓰지 않는다**(진짜 민감 권한 — 몇 분 늦는 건 감수).
- 알림 6종(전부 개별 토글): ① 아침 권장 금액(7·8·9시) ② 저녁 기록 안내(**오늘 지출 0건일 때만**, 20·21·22시) ③ **내일 나갈 돈**(저축·고정비 출금 전날 저녁) ④ 보류함 30일(딱 한 번) ⑤ 기간 결산(새 기간 첫날 아침) ⑥ 큰 지출 예상 달(기간 첫날 아침).
- **합치기 규칙**: 아침은 ①+⑤+⑥을 한 건(`BuildMorningNotificationUseCase`), 저녁은 ②+③을 한 건(`BuildEveningNotificationUseCase.merge`, 순수 함수·테스트 있음). 알림 여러 건이 연달아 오면 그것만으로 잔소리가 된다. ②③는 **서로 독립 토글**이라 기록 안내를 꺼도 ③만 받을 수 있다(`anyEveningEnabled`가 워커/예약 게이트).
- **제목 우선순위(저녁)**: 무지출 성공 축하 > 내일 출금(통장 확인=행동 필요) > 기록 안내. 축하해야 할 순간을 다른 안내가 잡아먹지 않게.
- **`NotificationContent.target`**: 합쳐지면 눌러서 갈 화면도 달라지므로 목적지를 내용과 함께 정한다(출금 안내만 오면 지출 입력 시트를 열지 않음). 워커는 `it.target`을 그대로 쓴다.
- **③ 내일 나갈 돈 프레이밍(중요)**: 예정 차감은 출금일 전에 이미 생활비에서 빠진 돈이라 **권장 금액이 바뀌지 않는다**(§4). 목적은 예산 경고가 아니라 **출금 통장 잔액 확인** 하나. 문구도 "이미 생활비에선 빼둔 돈이라 통장만 확인해두면 돼요"로 그 사실을 재확인한다. 출금일 판정은 `GetDeductionsDueOnUseCase`(순수, clamp 31→말일, 테스트 있음)이고 **내일이 속한 기간**으로 본다(기간 경계 안전).
- 기술: WorkManager OneTimeWork + 워커가 발송 후 스스로 다음 날 재예약, 부팅 후 `BootRescheduleReceiver`로 재예약. 채널 1개("하루 알림"), 기본 무음 IMPORTANCE_DEFAULT. 내용은 전부 기존 계산 파생 — **새 계산을 만들지 않는다**.
- **안 하는 것**: 초과 경고 알림(위젯에서 빨간 숫자를 뺀 것과 같은 이유), 입력 독촉, 마케팅·재참여 알림.
- 디버그 빌드에만 "지금 테스트 알림 보내기" 버튼 — 실제 발송과 **같은 UseCase**를 쓰므로 조건도 그대로 적용된다.

### 큰 지출 예상 달 안내 (구현됨, v1.4)

`GetBigSpendMonthNoticeUseCase` — 설·추석은 음력이라 **표로 관리**(2030년까지, 연 1회 갱신). 5월(가정의 달)·12월(연말)은 월 규칙. 파생이 아니라 **지식 주입**이라 데이터가 필요한 유일한 안내.

- 표시 2곳: ① 알림 ⑥(기간 첫날 아침) ② **월 탭** — 예산 요약 카드 아래 한 줄(💡). **CURRENT/FUTURE 모드에만** 표시(지난 달 결산엔 "미리 담아두면 편해요"가 무의미). 같은 UseCase 파생이라 알림과 문구가 항상 일치한다.
- 오늘 탭에는 넣지 않았다 — 배너가 이미 3종(가입 전 지출·결산·챌린지)이라 과밀.
- "주말이 많은 달"은 **안 함**: 한 달 주말은 8~10일로 편차가 작아 안내 가치가 낮고, 매달 말할 거리를 찾는 순간 잔소리 앱이 된다(§13).

### 설정 탭 "월 수입" (v1.4에서 동작 변경 ⚠️)

**설정 탭에서 월 수입을 바꾸면 이번 기간부터 실제로 적용된다.** 표시값도 프로필 기본값이 아니라 **이번 기간에 적용 중인 유효 예산**(`ObserveEffectiveMonthlyBudgetUseCase`)이다.

- 저장 시 **두 곳에 쓴다**: ① `SetMonthlyBudgetUseCase(현재 기간 anchorMonth, 금액)` → 이번 달 즉시 반영 + 레코드 없는 다음 달들도 이월로 따라옴 ② `BudgetProfile.monthlyIncome` → 레코드가 하나도 없을 때의 폴백 유지. 지난 기간은 각자 레코드가 있어 보존된다.
- ⚠️ **원래는 프로필 기본값만 갱신했다.** 그런데 §8 이월 규칙상 월별 레코드가 하나라도 생기면(월 탭·`+` 시트에서 한 번만 저장해도) 이월 조회가 항상 이겨서 **설정 탭 수정이 영구히 아무 데도 반영되지 않았다.** 설계 의도대로였지만 유저에겐 그냥 버그였고(실제로 반복 신고됨), 안내 문구를 붙이는 것으로는 해결되지 않아 동작 자체를 바꿨다.
- 특정 달만 다르게 하려면 월 탭에서 그 달을 수정한다(그 달 레코드가 우선). 두 입구의 역할: **설정 탭 = 지금부터 쭉 / 월 탭 = 이 달만.**
...
- **유닛 테스트**(`app/src/test`, junit4, 134개): 기간 계산·권장 금액·금액 이월·무지출 판정·카테고리 분류·리포트 페이스·알림(출금일 clamp·저녁 합치기·큰 지출 달). 계산 로직을 바꾸면 `./gradlew :app:testDebugUnitTest`로 먼저 확인한다. 회귀 방지용으로 남긴 케이스: 초과 5,400원(퍼센트 절삭), 아이스크림/아이크림(긴 키워드 승리), 8월 수정 시 7월 보존(이월), 기간 첫날·마지막 날 칩이 이웃 기간을 포함(조회 범위 회귀), 내역 묶기(날짜 그룹·정렬·기간 밖 제외), 막바지 여유분이 리포트 예상 잔액과 같은 숫자(미래 날짜 지출 포함).
