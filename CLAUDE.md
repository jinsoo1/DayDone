...
### 설정 탭 "월 수입(기본값)" 안내 (구현됨, v1.4)

월별 예산 레코드가 하나라도 있으면 이월 조회가 이겨서 **설정탭 기본값은 현재·미래 기간에 영향이 없다**(§8 설계대로, 버그 아님). 그런데 저장은 되니 "반영 안 됨"으로 읽힌다 → 수입 시트에 `NoticeBox`로 "이번 기간엔 월 탭에서 정한 예산({금액})이 쓰여요 / 이번 기간 금액은 월 탭에서 수정 / 기본값은 예산을 따로 정하지 않은 다음 기간부터" 안내. 판정은 값 비교가 아니라 **레코드 존재 여부**(`ObserveMonthlyBudgetRecordUseCase` — 폴백 없는 nullable 조회).
