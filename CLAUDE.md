...
- **유닛 테스트**(`app/src/test`, junit4, 108개): 기간 계산·권장 금액·금액 이월·무지출 판정·카테고리 분류·리포트 페이스·알림(출금일 clamp·저녁 합치기·큰 지출 달). 계산 로직을 바꾸면 `./gradlew :app:testDebugUnitTest`로 먼저 확인한다. 회귀 방지용으로 남긴 케이스: 초과 5,400원(퍼센트 절삭), 아이스크림/아이크림(긴 키워드 승리), 8월 수정 시 7월 보존(이월).
